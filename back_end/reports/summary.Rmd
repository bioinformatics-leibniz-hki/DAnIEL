...
```{r, include=FALSE, eval= paste0(statistics_dir, "stat.csv") %>% file.exists()}
stat_tbl <-
    ...
    "funguild" = {
      findings_tbl %>%
        dplyr::mutate(reference = case_when(
          ...
          ! is.na(citationSource) ~ stringr::str_glue("<div title='{citationSource}'>Reference</div>"),
          TRUE ~ ""
        )) %>%
        dplyr::mutate(relationship_html = stringr::str_glue(
          "<li>{growthForm} <i>{taxon}</i> is of guild {guild} ",
