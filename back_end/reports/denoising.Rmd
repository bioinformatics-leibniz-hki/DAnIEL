...
seq_df <- get_consensus_tbl(msa_path, tree)
...
```{r, include = FALSE, eval = denoising_params$denoising_method == "asv_dada2"}
dada2_l <-
...
- DADA2 key model parameters: `r {dada2_l %>% pluck("dada2_res_l", 1, "fwd_dada", "opts") %>% magrittr::extract(c("OMEGA_A", "OMEGA_C", "BAND_SIZE")) %>% purrr::map2(names(.), ~paste0(.y, "=", .x)) %>% as.character() %>% paste(collapse = ", ")}`
- Chimeras were removed using a *de novo* consensus approach
```
```{rmd, eval = denoising_params$denoising_method == "otu_pipits"}
- OTU profiling was used to generate representative sequences of qc reads [@pmid27570615]. Briefly, sequences with a minimum identity of 97% were clustered together after extracting the `r {denoising_params$its_region}` region.
...
- For the sake of clarity, only the `r {num_repseqs_showed}` most prevalent sequences were shown. The tree, however, was calculated based on all representative sequences
...
tooltip <-
  ...
  pull(zero_inflation) %>%
  summary() %>% {
    sprintf("Min: %.2f<br>Mean:%.2f<br>Median:%.2f<br>Max: %.2f",
            .[["Min."]], .[["Mean"]], .[["Median"]] , .[["Max."]])
  }
zero_inflation_plt <-
  zero_inflation_tbl %>%
