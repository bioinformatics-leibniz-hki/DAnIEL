...
params:
  ...
  project_dir: /sbidata/server/daniel/userdat/test_banocc/
...
```{r, eval = stat_bivariate_tbl %>% filter_significant() %>% dim() %>% purrr::pluck(1) > 0}
stat_bivariate_tbl %>%
  filter_significant() %>%
  dplyr::mutate(
    group_var = group_var %>%
...
```{r, eval = stat_aov_pre_tbl %>% filter_significant() %>% dim() %>% purrr::pluck(1) > 0}
stat_aov_pre_tbl %>%
  filter_significant() %>%
...
```{r, eval = stat_aov_post_tbl %>% filter_significant() %>% dim() %>% purrr::pluck(1) > 0}
stat_aov_post_tbl %>%
  filter_significant() %>%
  dplyr::arrange(feature) %>%
  dplyr::select(
...
```{r, eval = stat_cor_tbl %>% filter_significant() %>% dim() %>% purrr::pluck(1) > 0}
stat_cor_tbl %>%
  filter_significant() %>%
  dplyr::arrange(feature) %>%
    dplyr::select(
...
```{r, eval = "error" %in% colnames(stat_tbl) && stat_tbl %>% dplyr::filter(! is.na(error)) %>% dim() %>% purrr::pluck(1) > 0}
stat_tbl %>%
