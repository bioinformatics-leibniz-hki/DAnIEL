---
bibliography: "/sbidata/server/daniel/db/etc/daniel.bib"
csl: "/sbidata/server/daniel/db/etc/daniel.csl" 
output:
  html_document:
    theme: null
params:
...
<li><a href="#logs"><i class="fas fa-shield"></i> Logs</a></li>
...
> All results can be already accessed on the left sidebar.
...
Let's have a look at the first lines of the multiplexed raw read file `muxed_S4_S5_1.raw.fq.gz` in FASTQ format:
```{bash}
zcat $DANIEL_DB_DIR/example/reads/muxed_S4_S5_1.raw.fq.gz | head | cut -c 1-80
```
...
In the `reads` directory of `example.zip`, there are 8 files containing raw sequencing data from all 5 fecal samples from cancer patients:
```{bash}
...
- Raw read file names must follow the pattern `{sample_id}_{mate}.raw.fq.gz` where `{sample_id}` is a unique name of the sample and `{mate}` is either 1 or 2 and indicate the forward and reverse part of the paired read, respectively. Example for single samples: `sample1_1.raw.fq.gz`. Example for multiplexed samples: `muxed_S4_S5_2.raw.fq.gz`.

### Uploading own samples
...
We have two options to add samples from existing cohorts:

#### SRA Accessions
Every sequencing run stored in the SRA has a unique ID starting with `SRR` followed by some numbers.
...
We will select the project [hospital metagenome Targeted loci environmental (PRJNA505509)](https://www.ncbi.nlm.nih.gov/bioproject/?term=PRJNA505509) in the drop down menu `Existing projects` by clicking at the box and typing `hospital` to search.
Please click <i class="fas fa-database"></i> Projects on the sidebar to browse through all available projects.

We can subset samples from existing cohorts by any meta data variable as displayed at <i class="fas fa-database"></i> Projects.
This can be archived using a filter query.
We will enter the query `scientific_name == 'hospital metagenome' & total_spots > 30e3` to the text box `Filter query for existing projects`.
...
list(
  ...
  tidyr::unnest() %>%
  kable(caption = "Attributes available in all existing project to filter samples")
```

A click on <i class="fa fa-plus"></i>Add existing samples will add these sample to the project. The column `project` will be filled with the NCBI BioProject id.

![Upload local samples](tutorial/add_existing_samples.png)
...
In this example, we select the *Example* parameter set for all steps except for phylotyping.
Parameter sets except the default option are project specific and only appear in the drop down menu once created.

We can always click on <i class="fa fa-info-circle" style="color:#2a59a6"></i> at the right corner of each input element to display a short description.
...
> If you have ITS2 data, you have to create at least your own denoising parameter set!
...
- Denoising
  ...
  - Our example project covers the ITS2 sub region. Therefore we click on the radio button `ITS2`. It is not recommended to include denoised sequences only prevalent in one sample. Therefore we do not click the checkbox `Include singletons`.
- Phylotyping
  - Aim of this step: Taxonomic annotation of these denoised representative sequences
  - We do not create a specific parameter set here. We select the default set instead.
- Feature generation
  - Aim of this step: Normalization, filtering and pooling denoised abundance profiles
  - We change the `Taxonomic rank` from Species to Genus. Many fungi can not be classified up to the species level using the ITS region only. Pooling results at the genus level will also give us robust features prevalent in many samples.
- Analysis
  - Aim of this step: Creating statistics, Machine Learning, and correlation networks based on these features
  - Building correlation networks on all samples is meaningless in heterogeneous cohorts like ours. We want to investigate interaction in healthy individuals. Thus we enter the column name `disease` of the meta data table in the text box `Correlation grouping`

## <i class="fas fa-shield"></i> Logs
...
This is an optional feature of this webserver and only useful for debugging processes.
...
```{r}
readr::read_csv("/sbidata/server/daniel/userdat/example/qc/Selected_QC/final/excluded_samples.csv") %>%
...
> They will not be used if OTU profiling was selected in denoising parameter set.
...
|Bray-Curtis | UniFrac |
|:---: | :---: |
|![Bray-Curtis ordination](tutorial/features/ordination_bray.png){width=100%} | ![UniFrac ordination](tutorial/features/ordination_unifrac.png){width=100%}|
...
In this example, Principal Coordinates Analysis (PCoA) using the dissimilarities Bray-Curtis and UniFrac was performed.
...
In this example, the tree consist of genus, family, ..., kingdom nodes.
The distance of all child nodes to their parents is set to 1.
This is because sequence identity does not correlate with phylogenetic distance in the fungal kingdom using only the ITS region.

## <i class="fas fa-project-diagram"></i> Correlation Networks
Correlation is used to assess co-abundant features.

We used the filter query `project == 'healthy feces'` to only build the network on the healthy feces samples.
Here is the SparCC correlation network with default thresholds for effect size and significance [@pmid23028285; @pmid30169561]:
...
For example, *Saccharomyces* is significantly negatively correlated to *Dekkera* ($c_{SparCC}=-0.343, p = 0.005$).
The p values are obtained using permutation.
This is why we call them *pseudo p values*.
...
|Abundance of *Dekkera* by project | Abundance of *Saccharomyces* by project |
|:---: | :---: |
| ![Abundance of *Dekkera* by project](tutorial/statistics/box_dekkera.png){width="100%"} | ![Abundance of *Saccharomyces* by project](tutorial/statistics/box_saccharomyces.png){width="100%"} |
...
In our case, *Dekkera* in cancer feces samples is significantly differentially abundant comparing to healthy samples (Dunn's test, $p=3.9*10^{-4}$) and environmental ones (Dunn's test, $p=7.1*10^{-4}$).
